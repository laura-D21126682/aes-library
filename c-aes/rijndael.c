...
// Test Function: calling through ctypes test
int multiplyByThree(int num) { 
  return num * 3;
}

// Forward S-Box: Encryption lookup table
...
// Inverse S-Box: Decryption lookup table
...
// Mix-Columns: Multiply x 3 lookup table
...
// Invert Mix-Columns: Multiply x 9 lookup table
...
// Invert Mix-Columns: Multiply x 11 lookup table
...
// Invert Mix-Columns: Multiply x 13 lookup table
...
/**
  I had issues with submodules' shift_rows (and mix_columns) Python function 
  The Python results are returning what appears to be a shift-column instead of shift-row
  
  Here is an example unit test comparsion: 
  
  Input:                 | Python Output:       | C Output:
	[18, 245, 147, 19]     | [18, 27, 94, 184]    | [18, 245, 147, 19]
	[226, 27, 120, 161]    | [226, 17, 68, 19]    | [27, 120, 161, 226] 
	[44, 17, 94, 49]       | [44, 68, 147, 161]   | [94, 49, 44, 17] 
	[136, 68, 68, 184]     | [136, 245, 120, 49]  | [184, 136, 68, 68]

  To make my code work in the same way as the submodule I've created two BLOCK_ACCESS variables: column major and row major
...
void shift_rows(unsigned char *block) {
  ...
  unsigned char row1[4] = { BLOCK_ACCESS_COL(block, 1, 1), BLOCK_ACCESS_COL(block, 1, 2), BLOCK_ACCESS_COL(block, 1, 3), BLOCK_ACCESS_COL(block, 1, 0) }; // Shift each byte left 1 place
  ...
  unsigned char row3[4] = { BLOCK_ACCESS_COL(block, 3, 3), BLOCK_ACCESS_COL(block, 3, 0), BLOCK_ACCESS_COL(block, 3, 1), BLOCK_ACCESS_COL(block, 3, 2) }; // Shift each byte left 3 places

  // Each loop updates column n for each row
  for (int i = 0; i < 4; i++) {
    BLOCK_ACCESS_COL(block, 0, i) = row0[i];
    BLOCK_ACCESS_COL(block, 1, i) = row1[i];
    BLOCK_ACCESS_COL(block, 2, i) = row2[i];
    BLOCK_ACCESS_COL(block, 3, i) = row3[i];
  }
}
...
void mix_columns(unsigned char *block) {  
  ...
  for (int col = 0; col < 4; col++) {
    for (int row = 0; row < 4; row++) {
      BLOCK_ACCESS_COL(block, row, col) = temp_matrix[row][col];
    }
   }
}
...
void invert_shift_rows(unsigned char *block) {
  ...
  for (int i = 0; i < 4; i++) {
    BLOCK_ACCESS_COL(block, 0, i) = row0[i]; //  block 0,n = row 0,n
    BLOCK_ACCESS_COL(block, 1, i) = row1[i]; //  1,n = 1,n
    BLOCK_ACCESS_COL(block, 2, i) = row2[i]; //  2,n = 2,n
    BLOCK_ACCESS_COL(block, 3, i) = row3[i]; //  3,n = 3,n
  }
}
...
// Core key expansion function
void key_expansion_core(unsigned char *word, unsigned char rcon_index) {
  rotate_left(word); // Circular left shift
  map_to_sbox(word); // Map to S-Box
  word[0] ^= rcon[rcon_index]; // XOR first byte with RCon index 
}
...
unsigned char *expand_key(unsigned char *cipher_key) {
  ...
  unsigned char *expanded_keys = malloc(176); // allocates 176 bytes in memory for the expanded key
  
  if (!expanded_keys) { // check exists
    fprintf(stderr, "Failed to allocate memory for expanded keys!\n");
    return NULL;
  }

  // The first 16 bytes of the expanded key = cipher key (user supplied key)
  for (int i = 0; i < 16; i++){
  ...
  // continue expanding key untill reach 176 bytes
  while (bytes_generated < 176) {
    // copies the last 4 bytes of the expanded key to temp_word
    for(int i = 0; i < 4; i++) {
    ...
    //XORs temp_word with the previous 16 bytes
    for(unsigned char a = 0; a < 4; a++) {
      // Generates the next 4 bytes in the expanded_keys array
      // by XORing each byte of temp_word with a byte from the expanded_keys array (offset by 16)
      expanded_keys[bytes_generated] = expanded_keys[bytes_generated - 16] ^ temp_word[a]; 
      bytes_generated++;
    }
  }
...
unsigned char *aes_encrypt_block(unsigned char *plaintext, unsigned char *key) {
  ...
  unsigned char *output = (unsigned char *)malloc(sizeof(unsigned char) * BLOCK_SIZE); // allocate memory for output

  // Initialise output
  memcpy(output, plaintext, BLOCK_SIZE);

  // First Round
  add_round_key(output, expanded_key); // Add First 16 Bytes
  ...
  sub_bytes(output);
  shift_rows(output);
  add_round_key(output, expanded_key + 160); // the last key
...
unsigned char *aes_decrypt_block(unsigned char *ciphertext, unsigned char *key) {
  ...
  // First Round - Initialiase add_round_key with last round key from encryption process
  add_round_key(output, expanded_key + 160); // the last key
